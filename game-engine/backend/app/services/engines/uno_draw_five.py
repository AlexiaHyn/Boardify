...
class UnoDrawFivePlugin(GamePluginBase):
    def get_custom_actions(self):
        return {
            ...
            "draw_five": self._action_draw_five,
        }
    ...
    def _action_catch_uno(self, state, action):
        ...
        _draw_n(state, target, 4)
        state.log.append(_log(f"{catcher.name} caught {target.name}! Draw 4.", "effect", catcher.id))
    ...
    def _action_draw_five(self, state, action):
        player = self.get_player(state, action.playerId)
        if not player: return False, "Player not found", []
        if state.currentTurnPlayerId != player.id: return False, "Not your turn", []
        
        from app.services.engines.universal import _draw_n, _advance_turn
        _draw_n(state, player, 5)
        state.log.append(_log(f"{player.name} draws 5 cards", "action", player.id))
        _advance_turn(state)
        return True, "", ["drew_five"]

    def on_card_played(self, state, player, card):
        if len(player.hand.cards) == 1:
            state.metadata.setdefault("unoCalledBy", [])
        if card.subtype == "wild_draw_four":
...
def create_plugin(game_config):
    return UnoDrawFivePlugin("uno_draw_five", game_config)